        ...
        <table>
            ...
            @foreach($purchase_order_details as $purchase_order_detail)
                <tr>
                    ...
                    <td>
                        {{ explode('|#|', $purchase_order_detail->purpose)[0] }} <br><br>
                        <span style="font-size:10px">{{ (explode('|#|', $purchase_order_detail->purpose ?? '')[1] ?? null) ? 'MRS: ' . explode('|#|', $purchase_order_detail->purpose)[1] : '' }}</span>
                    </td>
                    ...
                    <td class="text-right">{{ number_format(($purchase_order_detail->price ?? 0) * ($purchase_order_detail->quantity ?? 1), 2, '.', ',') }}</td>
                    <td>
                        {{ explode('|#|', $purchase_order_detail->remarks)[0] }} <br><br>
                        <span style="font-size:10px">{{ (explode('|#|', $purchase_order_detail->remarks ?? '')[1] ?? null) ? 'MRS: ' . explode('|#|', $purchase_order_detail->remarks)[1] : '' }}</span>
                    </td>
                </tr>
            @endforeach
